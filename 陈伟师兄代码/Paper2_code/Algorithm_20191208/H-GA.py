...
#单位配送成本
delivery_cost_perMin=0.3
delivery_cost_perMeter=0.002
#线路配送里程
...
#订单分区处理时间=拣货时间+准备时间
data1=array([[0.6,1.2,0.6,1.1],[1.1, 0.7, 0, 1.1],[0.6, 0.7, 1.1, 0.9],[0.6, 0.9, 0.9, 0.8]])
set_up_time=0.15
data=[]
for i in data1:
    tem = []
    for j in i:
        if j>0:
            ...
            tem.append(0)
    data.append(tem)
data=insert(data,0,array([1,2,3,4]),axis=0)
...
batch_delivery_start_time = batch_arrive_time + array(batch_packingtime)
#批次订单等待时间=抵达后台时间-传送时间-拣货时间
batch_wait_time=array(batch_arrive_time)-4*transfer_time-array(data[1:].sum(axis=0))

#批次订单违约时间
batch_over_duetime = []
for j in list(array(batch_delivery_start_time) - array(op_duetime)):
    if j < 0:
        batch_over_duetime.append(0)
    else:
        batch_over_duetime.append(j)
#订单完成时间
batch_of_time=array(batch_delivery_start_time)+array(delivery_time) 
