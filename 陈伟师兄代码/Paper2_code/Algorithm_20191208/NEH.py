...
class NEH:
    ...
    def insert(self, data,transfer_time, total_time):
        ...
        for k in range(2, data.shape[1]):
            ...
            for i in range(k):
                if makespan_value(neh_data,transfer_time) > makespan_value(np.insert(neh_data, i, temp, axis=1),transfer_time):
...
def neh(data,transfer_time, draw=0):
    """
    ...
    data_neh = new.insert(data,transfer_time, total_time)
    end_time = time.time()
    ...
    if draw:
        ...
        plt.show()
    return data_neh,makespan_value(data[:, data_neh - 1],transfer_time)
