...
class GA_FSP_NEW:
    def __init__(self, data, pop_size=50, max_gen=800, Pc=0.65, Pm=0.2):
        self.data = data
    ...
    def fitness(self, pop,op_cost_perMin,op_duetime,over_duetime_cost_perMin,batch_packingtime,transfer_time):
        ...
        for i in range(self.pop_size):
            ...
            # fitness[i] = 1 / (sum(makespan(self.data[:, pop[i]])[-1, :])*op_cost_perMin)  # 合计完成时间*单位成本


            ######目标：合计订单拣选成本+违约成本##########
            #各批次订单抵达后台时间
            batch_arrive_time=np.array([i+transfer_time for i in (makespan(self.data[:, pop[i]],transfer_time)[-1, :])])
            # 各批次订单开始配送时间
            batch_delivery_start_time=batch_arrive_time+np.array(batch_packingtime)
             #违约时间
            batch_over_duetime=[]
            for j in list(np.array(batch_delivery_start_time)-np.array(op_duetime)):
                if j<0:
                    batch_over_duetime.append(0)
                else:
                    batch_over_duetime.append(j)
            # 总订单拣选成本+违约成本=合计订单开始配送时间*单位时间成本+合计违约时间*单位违约时间成本
            TC=((sum(batch_delivery_start_time)/10 * op_cost_perMin)+sum(batch_over_duetime)/10*over_duetime_cost_perMin)
            fitness[i] = 1 /TC
...
def ga_fsp_new(data,op_cost_perMin,op_duetime,over_duetime_cost_perMin,batch_packingtime,transfer_time,pop_size=50, max_gen=300, Pc=0.65, Pm=0.35, draw=222):
    """
    流水车间作业调度的改进遗传算法。
    ...
    for g in range(max_gen):
        fitness= new.fitness(pop,op_cost_perMin,op_duetime,over_duetime_cost_perMin,batch_packingtime,transfer_time)
    ...
    print("The minimum makespan: %s" % makespan_value(data[:, best_genetic],transfer_time)) #输出最大完成时间
    if draw != 222:
        import matplotlib.pyplot as plt
        from tool import gatt
        if int(str(draw)[0]) != 2:
            plt.figure(1)
            gatt(data[:, best_genetic],transfer_time)
            plt.show()
        if int(str(draw)[1]) != 2:
            plt.figure(2)
