...
#     sense=GRB.MINIMIZE)
# C_od_d=c_od_d*quicksum(z[i] for i in N)
MODEL.setObjective(
    c_d*(quicksum(D[i,g]*w[i,g] for i in N0 for g in N0))+f
    +c_od_d*quicksum(z[i] for i in N), 
    sense=GRB.MINIMIZE)
...
MODEL.addConstrs((z[i]==a[i]-l  for i in N),name="tw linearization")
...
# print("Current D_d Value:", (C_d-f)/c_d) #配送里程
