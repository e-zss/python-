...
def genInd(dataDict = dataDict):
    ...
    if choose_prob>=0.75: #质量最差 占25%
        ...
        from CVRPDV_Gurobi import route  #质量最优 占15%
        return route
...
def calOPCost(ind,M_max=4,c_op=c_op,t_convey=t_convey,
    ...
    #运行参数调整
    MODEL.Params.TimeLimit = 6 #设置求解时间最长为3s
    MODEL.Params.LogToConsole=0 #log记录不在控制台显示
    # 执行最优化
    MODEL.optimize()
    return round(MODEL.objVal,2)

def evaluate(ind, f=f,c_d=c_d,c2=8.0, c3=5.0,c4=1,c5=1):
    '''评价函数，返回解码后路径的总长度，
    f,c_d,c_d, c2分别为为单位派车成本、单位行驶成本、超载惩罚系数、时间窗惩罚系数'''
    ...
    C_op_od=calOPCost(ind)
    return (f*len(routes)+c_d*totalDistance + c2*loadPenalty(routes) + c3*timePenalty(ind,routes)+c4*cost_num_node+c5*C_op_od),
...
for eachRoute in distributionPlan:
    ...
    for i,j in zip(eachRoute[0::], eachRoute[1::]):
        totalDistance += calDist(dataDict['NodeCoor'][i], dataDict['NodeCoor'][j])
    C_d_Bat.append(totalDistance*c_d)
...
print(evaluate(bestInd,f=0,c_d=1,c2=0, c3=0,c4=0,c5=0)[0]) 
print('C_d：',evaluate(bestInd,c5=0)[0])
print('Total cost：',evaluate(bestInd,c5=1)[0])

 #f,c_d,c2,c3,c4分别为为单位派车成本、单位行驶成本、超载惩罚、时间窗惩罚、单线惩罚因子、拣选成本
...
print("IRS_G_G time used:",time()-st)
# 画出迭代图
Draw=0# 0-不绘制；1-绘制
if Draw==1:
