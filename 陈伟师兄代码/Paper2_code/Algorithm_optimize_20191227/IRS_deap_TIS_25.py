...
def genInd(dataDict = dataDict):
    ...
    # return getcind(dataDict = dataDict) 
...
def evaluate(ind, f=f,c_d=c_d,c2=8.0, c_od_d=5.0,c4=1,c5=1):
    '''评价函数，返回解码后路径的总长度，
...
print('C_d：',evaluate(bestInd,c5=0)[0])
print('Total cost：',evaluate(bestInd)[0])
print("IRS_deap_SSL time used:",time()-st)
...
def BtimePenalty(routes,c_od_d=5):
    
    '''计算每条线路的延迟成本'''
    timeArrangement = ODtimeTable(routes) # 对给定路线，计算到达每个客户的时间
    timeArrangement=[[0]+i+[0] for i in timeArrangement]
    # 索引给定的最迟到达时间
    desiredTime=[]
    for i in routes:
        tem=[]
        for j in i:
            tem.append(dataDict['Timewindow'][j][1])
        desiredTime.append(tem)
    
    # 如果最迟到达时间大于实际到达客户的时间，则延迟为0，否则延迟设为实际到达时间与最迟到达时间之差
    BtimeDelay=list(map(lambda x: np.array(x[0])-np.array(x[1]), zip(timeArrangement, desiredTime)))
    delaytime=[]
    for i in BtimeDelay:
        tem=[]
        for j in i:
            if j<0:
                ...
                tem.append(j)
        delaytime.append(tem)    
    Bat_time_Penalty=np.array([sum(i)*c_od_d for i in delaytime])
    return  Bat_time_Penalty
...
C_d_Bat=np.array(C_d_Bat)+ BtimePenalty(distributionPlan)

print("C_d_Bat：",C_d_Bat,sum(C_d_Bat))
...
for j in list(np.array(batch_delivery_start_time) - np.array([op_duetime[i] for i in order])):
    if j < 0:
        ...
        batch_over_duetime.append(j)
#批次订单完成时间
batch_of_time=np.around(batch_delivery_start_time)+np.around([ODtime[i] for i in order]) 
"""输出关键节点的时间"""
...
#各批次订单履行成本
TC_Bat=C_op_Bat+C_od_Bat+C_d_Bat

C_dod=BtimePenalty(distributionPlan,c_od_d=5)


print(ODtimeTable(distributionPlan, dataDict = dataDict))
