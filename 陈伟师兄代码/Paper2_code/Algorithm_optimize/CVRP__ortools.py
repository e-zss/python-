...
def create_data_model():
    ...
    def distance(Location):
        ...
        return Dis
    D=distance(Location)
    data['distance_matrix'] = list(D)
    """Computes travel time between two points"""
        ...
        for i in range(row):
            for j in range(row):
                if i==0:
                    ...
                    T[i,j]=D[i,j]/(v*(1-h2))
        return T
    v_trans=500 #500m/min
    h1=0.3 #caution intensity when depart from depot
    h2=0.15 #caution intensity in-transit
    T=traveltime(D,v_trans,h1,h2)
    data['time_matrix'] = list(T)

    """ add time windows data"""
    e = list(rawdata.iloc[:, 4])
    l = list(rawdata.iloc[:, 5])
    time_windows = []
    for i in range(len(rawdata)):
        time_windows.append((e[i], l[i]))
    data['time_windows'] = list(time_windows)
    
    """add num_vehicles"""
    num_vehicles=20
    data['num_vehicles'] = num_vehicles

    """add demand of each location"""
    q = list(rawdata.iloc[:, 3])
    data['demands'] = q

    """add service time of each location"""
    s=  [int(i*0.5) for i in q]  #s[i]
    data['service_time'] = s
    """add num_capacities of each vehicle"""
    capacities = []
